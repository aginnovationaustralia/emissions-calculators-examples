...
class PostDairyRequest(BaseModel):
    ...
    def production_system_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Non-irrigated Crop', 'Irrigated Crop', 'Irrigated Pasture', 'Non-irrigated Pasture']):
            raise ValueError("must be one of enum values ('Non-irrigated Crop', 'Irrigated Crop', 'Irrigated Pasture', 'Non-irrigated Pasture')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in dairy (list)
        _items = []
        if self.dairy:
            for _item_dairy in self.dairy:
                if _item_dairy:
                    _items.append(_item_dairy.to_dict())
            _dict['dairy'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "dairy": [PostDairyRequestDairyInner.from_dict(_item) for _item in obj["dairy"]] if obj.get("dairy") is not None else None,
            "vegetation": [PostDairyRequestVegetationInner.from_dict(_item) for _item in obj["vegetation"]] if obj.get("vegetation") is not None else None
        })
