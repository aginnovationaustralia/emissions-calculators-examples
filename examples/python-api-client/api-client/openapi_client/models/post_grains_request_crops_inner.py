...
class PostGrainsRequestCropsInner(BaseModel):
    """
    Input data required for a single Grains enterprise
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Unique identifier for this Grains activity")
    type: StrictStr = Field(description="Crop type. Note that the following crop types are now deprecated, the relevant full calculator should be used instead: 'Cotton', 'Rice', 'Sugar Cane'")
    ...
    production_system: StrictStr = Field(description="Production system of this crop. Note that the following production systems are now deprecated, the relevant full calculator should be used instead: 'Cotton', 'Rice', 'Sugar cane'", alias="productionSystem")
    average_grain_yield: Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]] = Field(description="Average grain yield, in t/ha (tonnes per hectare)", alias="averageGrainYield")
    ...
    __properties: ClassVar[List[str]] = ["id", "type", "state", "productionSystem", "averageGrainYield", "areaSown", "nonUreaNitrogen", "ureaApplication", "ureaAmmoniumNitrate", "phosphorusApplication", "potassiumApplication", "sulfurApplication", "rainfallAbove600", "fractionOfAnnualCropBurnt", "herbicideUse", "glyphosateOtherHerbicideUse", "electricityAllocation", "limestone", "limestoneFraction", "dieselUse", "petrolUse", "lpg"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Wheat', 'Barley', 'Maize', 'Oats', 'Rice', 'Sorghum', 'Triticale', 'Other Cereals', 'Pulses', 'Tuber and Roots', 'Peanuts', 'Sugar Cane', 'Cotton', 'Hops', 'Oilseeds', 'Forage Crops', 'Lucerne', 'Other legume', 'Annual grass', 'Grass clover mixture', 'Perennial pasture']):
            raise ValueError("must be one of enum values ('Wheat', 'Barley', 'Maize', 'Oats', 'Rice', 'Sorghum', 'Triticale', 'Other Cereals', 'Pulses', 'Tuber and Roots', 'Peanuts', 'Sugar Cane', 'Cotton', 'Hops', 'Oilseeds', 'Forage Crops', 'Lucerne', 'Other legume', 'Annual grass', 'Grass clover mixture', 'Perennial pasture')")
        return value
    ...
    def production_system_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Non-irrigated crop', 'Irrigated crop', 'Sugar cane', 'Cotton', 'Horticulture']):
            raise ValueError("must be one of enum values ('Non-irrigated crop', 'Irrigated crop', 'Sugar cane', 'Cotton', 'Horticulture')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "averageGrainYield": obj.get("averageGrainYield"),
